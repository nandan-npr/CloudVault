  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--background)] px-6 py-10 text-[var(--cv-text)]">
      <section className="w-full max-w-md rounded-[28px] border border-[var(--cv-border)] bg-[var(--cv-surface-card)] p-8 shadow-sm">
        ...
        <h1 className="mt-2 text-3xl font-semibold text-[var(--cv-text)]">Choose a new password</h1>
        {!token ? (
          <p className="mt-4 text-sm text-red-700">This reset link is invalid or incomplete.</p>
        ) : (
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            <label htmlFor="password" className="block text-sm font-medium text-[var(--cv-text-muted)]">
              New password
              <input
                id="password"
                type="password"
            ...
            <label htmlFor="confirmPassword" className="block text-sm font-medium text-[var(--cv-text-muted)]">
